...
import Agent.TD3 as TD3
import Agent.DDPG as DDPG
from utils.visu.visu_results2 import *
...
if __name__ == "__main__":
	...
	episode_critic_error = 0
	episode_actor_error = 0
	...
	for t in range(int(args.max_timesteps)):
		...
		if t >= args.start_timesteps:
			...
			episode_critic_error += policy.critic_loss
			episode_actor_error += policy.actor_loss

		if done: 
			# +1 to account for 0 indexing. +0 on ep_timesteps since it will increment +1 even if done=True
			print(f"Total T: {t+1} Episode Num: {episode_num+1} Episode T: {episode_timesteps} Reward: {episode_reward:.3f}")

			CriticError.append(episode_critic_error)
			PolicyError.append(episode_actor_error)
		...
		if (t + 1) % args.eval_freq == 0:
			...
			if args.save_model: 
				policy.save(args.env, evaluations[-1],args.team_name)
	exploit_policy_loss_full(PolicyError)
	exploit_critic_loss_full(CriticError)
	exploit_reward_full(AllReward)
